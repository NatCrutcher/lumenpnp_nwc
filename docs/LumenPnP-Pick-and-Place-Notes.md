...
Set the bottom-camera roaming radius to 28.5 mm (Machine Setup → Cameras → Bottom → Position tab; set 2026-08-30, commits `f15f459`/`b1c7d62`, [issue #8](https://github.com/NatCrutcher/lumenpnp_nwc/issues/8)). The shipped 0.0 disables OpenPnP's footprint-based capture sizing entirely (`NoCameraRoaming`). Budget from measured clearance: the part-bottom / camera-focus plane sits 10 mm above the build plate, so the 45.0 mm plate opening and the 19.5 mm light-ring diffuser ID are below the part travel plane and constrain nothing; the nearest obstruction at camera Z is the fiducial supports at 30 mm from the camera axis → 30.0 − 1.0 mm N24 pick tolerance − 0.5 mm margin = 28.5 mm. See the [clearance diagram](img/roaming-radius-clearance.svg) and the write-up in [OpenPnP-Vision-Concepts.md](OpenPnP-Vision-Concepts.md#auto-derived-sizes). Two cautions: within 29.5 mm of the camera, planned moves between captures travel **at camera Z** without retracting to safe Z (by design) and planned part edges may legally reach 29.5 mm — 0.5 mm from the fiducial supports — so the first job that plans off-center captures of a large part deserves single-stepping; and the radius bounds *planned* part extent only — a manual jog can still drive a large part's edge into the fiducial supports at camera Z, so don't hand-jog large parts near the camera at low Z.
